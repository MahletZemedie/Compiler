...
{int i;
    ...
    temp_num = -3;  // just a trick so that the two temp created in the next two lines will not named TEMP#1 and TEMP#2
...
SYMTBL symtbl_install(char *sym, int stype, int dtype, int scope)
{
    ...
    if (stype == SYMBOL_ID || stype == SYMBOL_PARA || stype == SYMBOL_TEMP)   // *** NEW ***
       entry->local_var_index = local_var_num++;                              // *** NEW ***
...
{int scope_level;
    ...
    for (scope_level = head_scope_link->scope;
         head_scope_link && (head_scope_link->scope ==  scope_level); 
